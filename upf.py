...
def epfformat(mode=None, filename=None):
    ...
    each of axes stands for rotating and tilting
...
class polefigure:
    ...
    def epfplot(self, ifig, mode, cmode, levels,rot):
        ...
        if True:
            print 'ifig=',ifig
            fact = 2.
            figsiz = (len(self.grid)* 2.8 * fact, 1.*2.*fact)
            fig = plt.figure(ifig,   figsize=figsiz) #polar axes figure
            figc= plt.figure(ifig+10, figsize=figsiz)
            figr= plt.figure(ifig+20, figsize=figsiz)
            fig.clf(); figc.clf(); figr.clf() #clear the canvas.
            nrow = len(self.grid)

            ## loop over each of pole figures
            for ip in range(len(self.grid)):
                ...
                #                                    #-> cartensian
                x_r = RR*np.cos(PHIR); y_r = RR*np.sin(PHIR)
                ...
                max_khi = 80#upto 80 degree is convetional to me...
                ...
                # _ max khi outline on the cartesian
                x0, y0 = circle(r=r0[0])
                axc.plot(x0, y0, 'k')
                ########################################
                ...
                # print pfr.shape, phi.shape, rr.shape
                # print pf.shape, phi.shape, r.shape
                cntr = axr.contour(
                ...
                for i in range(len(tcolorsc)):
                    ...
                    if levels==None or ip==len(pole)-1:
                        x0, y0 = 1.3, 1. - i * 0.2
                        axc.plot(x0, y0, marker='o', ms=6./len(self.grid),
                                 #color=cc,
                                 mfc=cc,
                                 ls='None', mec='None', 
                                 markeredgewidth=0.7/len(self.grid)
                    ...
                    x4, y4 = -0.05, 1.05
                    axc.text(x=x4, y=y4,
                             s='RD', fontsize = 11.*fact/len(self.grid))
                    x5, y5 = 1.05, 0.
                    axc.text(x=x5, y=y5,
                             s='TD', fontsize = 11.*fact/len(self.grid))
                    axc.set_axis_off()
                    #axr.set_ylim(0,1.7) #-->radius lim
                    axc.set_xlim(-1.7,1.7)
                    axc.set_ylim(-1.7,1.7)
                    #axp.set_ylim(0,1.7)
                pass
            #fig.savefig('fig_ax.pdf')
            figc.savefig('fig_axc.pdf')
            figc.savefig('fig_axc.eps')
            figr.savefig('fig_axr.pdf')
            figr.savefig('fig_axr.eps')
            ...
            ## For experimental pole figure no binning process below is necessary
            return
    ...
    def pf(self, pole=[[1,0,0],[1,1,0],[1,1,1]], mode='contour',
        ...
        ** Exemplary cmodes:
        ...
        if mode=='dot':
            ...
            ## polar proejction option in subplot. Therefore, at the time
            ...
            if axes!=None:
                raise IOError, "'im' mode does not support imposition of axes"
            fig = plt.figure(ifig, figsize=figsize)
    ...
    def ipf(self,pole=None,color='k'):
        """
    ...
    def core(self, pole=None, proj='pf', csym=None, ssym=None,
        ...
        """
        xy = []; POLE = [];
        if csym!='cubic' and csym!='hexag':
            print "Other symmetries than cubic or hexag  is not prepared yet"
            raise IOError
        if proj!='pf' and proj!='ipf':
            print "Other modes of projection than pf and ipf is not prepared yet"
            raise IOError
        if agrain==None:
        ...
        ## inverse pole figure
            ...
            if abs(pole[0]**2+pole[1]**2+pole[2]**2-1)>0.1**6:
                print "The pole must be one of principal axes of the sample"
                print "It should be among [1,0,0], [0,1,0], [0,0,1]"
                print "current pole is as below\n", pole
                raw_input()
                raise IOError
            npeq = [pole] ## Note it is sample axis vector!
            #equivalent pole calculatation is deffered to next for in block
    ...
    def dotplot(self, pole=None, ifig=None,
                alpha=1.0, color='k', 
                proj='ipf', csym='cubic',
        ...
        if ifig==None: pass
            ...
            fact=2.
            figsize=(2*fact, 1.*fact)
            fig = plt.figure(ifig, figsize=figsize)
            ax = fig.add_subplot(111); #ax.set_frame_on(False)
            ax.set_axis_off(); ax.set_aspect('equal')
            ax.text(x=-0.05, y= 1.05, s='TD', fontsize=4.*fact)
            ax.text(x= 1.05, y= 0.  , s='RD', fontsize=4.*fact)
            if proj=='pf':
